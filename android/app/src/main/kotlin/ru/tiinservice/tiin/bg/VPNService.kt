...
import ru.tiinservice.tiin.constant.PerAppProxyMode
import ru.tiinservice.tiin.ktx.toIpPrefix
...
//        service.fileDescriptor?.close()
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                ...
                val underlyingNetworks = networks.filter { network ->
...
//        service.sendNotification(notification)
    ...
    private val dnsNoiseDomains = listOf(
        "clients3.google.com",
        "play.googleapis.com",
    ...
    private val dnsNoiseHandler = Handler(Looper.getMainLooper())
    private var dnsNoiseRunnable: Runnable? = null

    private fun startDnsNoise() {
        dnsNoiseRunnable = object : Runnable {
            override fun run() {
                try {
                    val domain = dnsNoiseDomains.random()
                    java.net.InetAddress.getByName(domain)
                } catch (_: Exception) {
                }
                dnsNoiseHandler.postDelayed(this, 30000)
            }
        }
        dnsNoiseHandler.postDelayed(dnsNoiseRunnable!!, 30000)
    }

    private fun stopDnsNoise() {
        dnsNoiseRunnable?.let { dnsNoiseHandler.removeCallbacks(it) }
        dnsNoiseRunnable = null
    }
}